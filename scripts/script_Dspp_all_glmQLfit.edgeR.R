...
for (t in time){
	...
	for (i in colnames(sp.contrasts)){
		...
		pTable.i.t<-assign(paste("QL.pTable.sp",i,t,sep="."),pTable)
	}
...
QL.pTable.sp.Clade.5 <- as.data.frame(setNames(replicate(4,numeric(0), simplify = F), 
			c("FBgn_mel","CG","logFC","FDR")))
QL.pTable.sp.Clade.50 <- as.data.frame(setNames(replicate(4,numeric(0), simplify = F), 
			c("FBgn_mel","CG","logFC","FDR")))

for (i in colnames(sp.contrasts)){
	ptable.5<-(paste("QL.pTable.sp",i,5,sep="."))
	ptable.50<-(paste("QL.pTable.sp",i,50,sep="."))
	if(i!="Par"){
		QL.pTable.sp.Clade.5<-rbind(QL.pTable.sp.Clade.5,
					get(ptable.5)[which(!(get(ptable.5)$FBgn_mel%in%QL.pTable.sp.Clade.5$FBgn_mel)),])
		QL.pTable.sp.Clade.50<-rbind(QL.pTable.sp.Clade.50,
					get(ptable.50)[which(!(get(ptable.50)$FBgn_mel%in%QL.pTable.sp.Clade.50$FBgn_mel)),])
	}
}
